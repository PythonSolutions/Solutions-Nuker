...
def selector(token, users):
    while True:
        try:
            response = requests.post(f'https://discordapp.com/api/v9/users/@me/channels', proxies={"ftp": f'{proxy()}'}, headers=getheaders(token), json={"recipients": users})
...
def GcSpammer(token):
    ...
    if secondchoice == 1:
        ...
        recipients = input(
            f'{Fore.GREEN}[{Fore.CYAN}>>>{Fore.GREEN}] {Fore.RESET}Input the users you want to create a groupchat with (separate by , id,id2,id3): {Fore.LIGHTRED_EX}')
